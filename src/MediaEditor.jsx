...
export default function MediaEditor() {
  ...
  // ======= EXPORT VIDEO (fast — accelerated playback) =======
  const handleExportVideo = async () => {
    ...
    try {
      ...
      try {
        ...
        console.warn("Audio not captured:", audioErr.message);
      }
      ...
      for (const [mime, ext] of tryMimes) {
        if (MediaRecorder.isTypeSupported(mime)) {
          mimeType = mime; fileExt = ext; break;
        }
      }

      setExportStatus(`Gravando ${fileExt.toUpperCase()}... (acelerado)`);
      ...
      // Speed up playback (audio will be distorted but we get faster export)
      v.muted = true; // mute during fast export to avoid distortion
      v.playbackRate = 8; // 8x speed — a 1min video exports in ~8 seconds
      await v.play();
      ...
      const renderLoop = () => {
        ...
        // Frame border in export
        if (frameBorder !== "none" && frameBorderWidth > 0) {
          const fbColors = { white: "#fff", gray: "#888", black: "#000", custom: frameBorderColor };
          ctx.save();
          ctx.strokeStyle = fbColors[frameBorder] || "#fff";
          ctx.lineWidth = frameBorderWidth * 2;
          if (frameBorderRadius > 0) {
            ctx.beginPath();
            ctx.roundRect(0, 0, canvas.width, canvas.height, frameBorderRadius);
            ctx.stroke();
          } else {
            ctx.strokeRect(0, 0, canvas.width, canvas.height);
          }
      ...
      await new Promise((r) => {
        const checkEnd = () => {
          if (exportDone) { r(); return; }
          setTimeout(checkEnd, 200);
        };
        v.addEventListener("ended", () => { exportDone = true; v.pause(); recorder.stop(); r(); }, { once: true });
        checkEnd();
      });
      ...
      setExportStatus("");
      notify(`Vídeo ${fileExt.toUpperCase()} exportado!`);

    } catch (err) {
      setExporting(false);
      setExportStatus("");
      if (videoRef.current) { videoRef.current.playbackRate = 1; videoRef.current.muted = muted; }
      notify("Erro: " + err.message, "error");
    }
  };
          ...
          <div style={S.panel}>
            ...
            {mediaType === "video" && (
              <button
                ...
                disabled={exporting}
              >
                {exporting ? `⏳ ${exportProgress}%` : "🎬 Exportar Vídeo"}
              </button>
            )}
            {exporting && (
              <>
            ...
            <p style={S.help}>
              ...
              Vídeo: grava com edições + áudio (rápido!)
            </p>
          </div>
        </div>
