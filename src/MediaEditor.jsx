...
export default function MediaEditor() {
  ...
  }, [covers, attenuations, logoSize, logoOpacity, logoTint, logoBorder, logoBorderWidth, logoAnimation, logoAnimDuration, mediaType, attenuateWatermark]);

  useEffect(() => {
    if (mediaLoaded) drawFrame();
  }, [covers, attenuations, logoSize, logoOpacity, logoTint, logoBorder, logoBorderWidth, mediaLoaded, drawFrame]);
  ...
  // ======= EXPORT VIDEO =======
  const lastWebmBlobRef = useRef(null);
  const [webmReady, setWebmReady] = useState(false);
  ...
  const handleExportVideo = async () => {
    ...
    setExporting(true);
    setExportProgress(0);
    setExportStatus("Gravando vídeo...");
    setWebmReady(false);
    ...
    try {
      ...
      const renderLoop = () => {
        ...
        if (logoImgRef.current) {
          ...
          const baseX = logoXRef.current, baseY = logoYRef.current;
          let animAlpha = 1, aX = 0, aY = 0, aScale = 1, aRot = 0;
          if (logoAnimation !== "none") {
            ...
            switch (logoAnimation) {
              case "fadeIn": animAlpha = ease; break;
              ...
              case "zoomIn": aScale = 0.1+ease*0.9; animAlpha = ease; break;
              case "spin": aRot = (1-ease)*Math.PI*2; aScale = 0.5+ease*0.5; break;
              case "bounce": { const be = t<0.5?4*t*t*t:1-Math.pow(-2*t+2,3)/2; aY=(1-be)*-150; aScale=0.8+be*0.2; break; }
              case "pulse": { animAlpha=ease; const pt=Math.min(1,el/(logoAnimDuration*0.5)); aScale=1+Math.sin(pt*Math.PI*3)*0.15*(1-pt); break; }
            }
          }
          ctx.save();
          ctx.globalAlpha = (logoOpacity/100)*animAlpha;
          const cx2 = baseX+aX+(logoSize*aScale)/2, cy2 = baseY+aY+(lh*aScale)/2;
          ctx.translate(cx2, cy2);
          if (aRot !== 0) ctx.rotate(aRot);
        ...
        if (v.duration) setExportProgress(Math.round((v.currentTime / v.duration) * 95));
      ...
      lastWebmBlobRef.current = webmBlob;
  ...
  const handleConvertMp4 = async () => {
    if (!lastWebmBlobRef.current) return;
    setConvertingMp4(true);
    setExportStatus("Carregando conversor...");
    setExportProgress(0);

    try {
      const ffmpeg = await loadFFmpeg();
      setExportStatus("Convertendo para MP4 (pode demorar)...");

      ffmpeg.on("progress", ({ progress }) => {
        setExportProgress(Math.round(progress * 100));
      });

      const webmData = await fetchFile(lastWebmBlobRef.current);
      ...
      setExportStatus("");
      setExportProgress(100);
      notify("MP4 exportado!");
    } catch (err) {
      setConvertingMp4(false);
      setExportStatus("");
      notify("Erro na conversão MP4: " + err.message, "error");
    }
  };
      ...
      <div style={S.main}>
        ...
        <div style={S.sidebar}>
          ...
          <div style={S.panel}>
            ...
            {mediaType === "video" && (
              <>
                <button
                  ...
                  disabled={exporting || convertingMp4}
                >
                  {exporting ? `⏳ Gravando... ${exportProgress}%` : "🎬 Exportar Vídeo (WebM) — Rápido"}
                </button>

                {webmReady && !convertingMp4 && (
                  <button
                    style={{ ...S.btn, marginBottom: 6, background: "linear-gradient(135deg, #f59e0b, #d97706)", border: "none", color: "#fff" }}
                    onClick={handleConvertMp4}
                  >
                    🔄 Converter para MP4 (mais lento)
                  </button>
                )}

                {convertingMp4 && (
                  <button style={{ ...S.btn, marginBottom: 6, background: "#333", border: "none", color: "#999", cursor: "not-allowed" }} disabled>
                    ⏳ Convertendo MP4... {exportProgress}%
                  </button>
                )}
              </>
            )}
            {(exporting || convertingMp4) && (
              <>
            ...
            <p style={S.help}>
              WebM: baixa em segundos, funciona em quase tudo<br />
              MP4: conversão no navegador (mais demorado)
            </p>
          </div>
        </div>
