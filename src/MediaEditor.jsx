...
export default function MediaEditor() {
  ...
  const handleExportVideo = async () => {
    ...
    try {
      ...
      // Save blob for optional MP4 conversion
      ...
      setExportStatus("");
      notify(`WebM exportado! (${(blob.size / 1024 / 1024).toFixed(1)}MB) — Use o botão MP4 para PowerPoint`);
  ...
  // ======= FFMPEG: CONVERT WEBM → MP4 (separate step) =======
  const handleConvertMp4 = async () => {
    if (!lastWebmBlobRef.current) return;

    setConverting(true);
    setConvertProgress(0);
    setExportStatus("Carregando conversor MP4...");
    ...
    try {
      ...
      ffmpeg.on("progress", ({ progress }) => {
        ...
        setExportStatus(`Convertendo... ${pct}%`);
      });
      ...
      setExportStatus("");
      notify(`MP4 pronto! (${(mp4Blob.size / 1024 / 1024).toFixed(1)}MB) — Compatible com PowerPoint`);

    } catch (err) {
      setConverting(false);
      setExportStatus("");
      notify("Erro na conversão: " + err.message, "error");
    }
  };
          ...
          <div style={S.panel}>
            ...
            {mediaType === "video" && (
              <>
                {/* Step 1: Record WebM */}
                <button
                  style={{ ...S.btn, marginBottom: 6, background: exporting ? "#333" : "linear-gradient(135deg, #22c55e, #16a34a)", border: "none", color: "#fff", cursor: (exporting || converting) ? "not-allowed" : "pointer" }}
                  onClick={handleExportVideo}
                  disabled={exporting || converting}
                >
                  {exporting ? `⏳ Gravando... ${exportProgress}%` : "🎬 1. Gravar Vídeo com Áudio"}
                </button>

                {/* Step 2: Convert to MP4 (appears after WebM is ready) */}
                {webmReady && !exporting && (
                  <button
                    style={{ ...S.btn, marginBottom: 6, background: converting ? "#333" : "linear-gradient(135deg, #f59e0b, #d97706)", border: "none", color: "#fff", cursor: converting ? "not-allowed" : "pointer" }}
                    onClick={handleConvertMp4}
                    disabled={converting}
                  >
                    {converting ? `⏳ Convertendo... ${convertProgress}%` : "📎 2. Converter para MP4 (PowerPoint)"}
                  </button>
                )}
              </>
            )}
            {(exporting || converting) && (
              <>
                <div style={{ width: "100%", height: 6, background: "#252535", borderRadius: 3, overflow: "hidden", marginBottom: 4 }}>
                  <div style={{ width: `${exporting ? exportProgress : convertProgress}%`, height: "100%", background: exporting ? "linear-gradient(90deg, #6366f1, #22c55e)" : "linear-gradient(90deg, #f59e0b, #ef4444)", borderRadius: 3, transition: "width 0.3s" }} />
                </div>
                {exportStatus && (
            ...
            <p style={S.help}>
              1. Gravar: captura o vídeo com edições + áudio<br />
              2. Converter: transforma em MP4 para PowerPoint<br />
              <span style={{ color: "#f59e0b" }}>A conversão carrega ~25MB na 1ª vez</span>
            </p>
          </div>
        </div>
