...
export default function MediaEditor() {
  ...
  const handleExportVideo = async () => {
    ...
    try {
      ...
      // Play video from beginning and render each frame to canvas
      v.currentTime = 0;
      await new Promise((r) => { v.onseeked = r; });

      v.play();

      // Render loop while video plays
      const renderLoop = () => {
        if (v.ended || v.paused) {
          recorder.stop();
          return;
        }
        // Draw frame with all overlays
        const ctx = canvas.getContext("2d");
        ...
        // Update progress
        if (v.duration) {
          setExportProgress(Math.round((v.currentTime / v.duration) * 100));
        }
      ...
      notify("Vídeo exportado com sucesso!");
          ...
          <div style={S.panel}>
            ...
            {mediaType === "video" && (
              <button
                ...
                disabled={exporting}
              >
                {exporting ? `⏳ Processando... ${exportProgress}%` : "🎬 Exportar Vídeo (WebM)"}
              </button>
            )}
            {exporting && (
              <div style={{ width: "100%", height: 6, background: "#252535", borderRadius: 3, overflow: "hidden", marginBottom: 6 }}>
            ...
            <p style={S.help}>
              ...
              Vídeo: processa todos os frames com as edições
            </p>
          </div>
        </div>
