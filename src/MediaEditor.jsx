...
export default function MediaEditor() {
  ...
  const drawFrame = useCallback(() => {
    ...
    if (logoImgRef.current) {
      ...
      if (logoTint) {
        // Draw logo to offscreen canvas, then tint it
        const offCanvas = document.createElement("canvas");
        offCanvas.width = logoSize;
        offCanvas.height = h;
        const offCtx = offCanvas.getContext("2d");
        offCtx.drawImage(logoImgRef.current, 0, 0, logoSize, h);
        // Apply color with multiply-like effect: fill with tint, then use source-in
        offCtx.globalCompositeOperation = "source-in";
        offCtx.fillStyle = logoTint;
        offCtx.fillRect(0, 0, logoSize, h);
        ctx.drawImage(offCanvas, logoXRef.current, logoYRef.current);
      } else {
  ...
  }, [covers, attenuations, logoSize, logoOpacity, logoTint, mediaType, attenuateWatermark]);

  useEffect(() => {
    if (mediaLoaded) drawFrame();
  }, [covers, attenuations, logoSize, logoOpacity, logoTint, mediaLoaded, drawFrame]);
      ...
      const renderLoop = () => {
        ...
        if (logoImgRef.current) {
          ...
          if (logoTint) {
            ...
            offCanvas.height = lh;
            const offCtx = offCanvas.getContext("2d");
            offCtx.drawImage(logoImgRef.current, 0, 0, logoSize, lh);
            offCtx.globalCompositeOperation = "source-in";
            offCtx.fillStyle = logoTint;
            offCtx.fillRect(0, 0, logoSize, lh);
            ctx.drawImage(offCanvas, logoXRef.current, logoYRef.current);
          } else {
            ctx.drawImage(logoImgRef.current, logoXRef.current, logoYRef.current, logoSize, lh);
          }
          ctx.restore();
        }
